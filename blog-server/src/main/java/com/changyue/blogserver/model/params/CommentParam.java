...
public class CommentParam implements InputConvert<Comments> {
    ...
    public CommentParam(Integer commentUserId, @NotBlank(message = "评论内容不能为空") @Size(max = 1023, message = "评论内容的字符长度不能超过 {max}") String content, @Min(value = 1, message = "Post id must not be less than {value}") Integer postId, @Min(value = 0, message = "PostComment parent id must not be less than {value}") Long parentId) {
